...
class ChatBot extends Component {
  ...
  handleEnd() {
    const { previousSteps, currentPath } = this.state;
    ...
    if (this.props.handleEnd) {
      ...
      this.props.handleEnd({ renderedSteps, steps, values, currentPath:oldPath });
    }
  }

  niceFunctie(arrayVanKeys){
    arrayVanKeys.forEach((key) => {
  ...
  triggerNextStep(data) {
    ...
    const currentPath = this.props.mainOptions.find(key => key === currentStep.trigger);
    if(currentPath){
      this.setState({
        currentPath
      })
